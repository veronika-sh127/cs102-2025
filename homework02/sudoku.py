...
"""Модуль решения судоку: чтение, решение, генерация, проверка"""
T = tp.TypeVar("T")
...
def create_grid(puzzle: str) -> tp.List[tp.List[str]]:
    """Генерация поля для решения судоку"""
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    used_digits = row_digits | col_digits | block_digits
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for i in range(9):
        if set(get_row(solution, (i, 0))) != values:
            return False
    for j in range(9):
        if set(get_col(solution, (j, 0))) != values:
            return False
    for block_row in range(0, 9, 3):
        for block_col in range(0, 9, 3):
            if set(get_block(solution, (block_row, block_col))) != values:
                return False
    return True
